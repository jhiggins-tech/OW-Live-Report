# OW-Live-Report V2 — Greenfield Build

> **Status update (2026-05):** V2 is now the site. The V1 → V2 cutover
> shipped per [`v1-to-v2-cutover.md`](./v1-to-v2-cutover.md); V1 is
> archived at tag `v1-final`. This doc is preserved for historical
> context — paths that say `/v2/` or `docs/v2/` now mean root (`/`,
> `docs/`).

## Context

V1 was a static site rendered by ~5,300 lines of PowerShell modules: at build time, `run-report.ps1` pulled from InfluxDB + OverFast, transformed snapshots, and baked ~20MB of JSON into `docs/`. This pipeline was brittle (Windows CI, race between build & Pages serving, stale `docs/`-on-`main` deploys — the Melrose-missing incident). It also meant data was only as fresh as the last CI run.

V2 is a greenfield rebuild that:
- Originally hosted at `/v2/` (built into `docs/v2/`) alongside V1 — V1 site was untouched until the cutover.
...
- Shipped an MVP first, with a tracked checklist of V1 features deferred to V2.1. Cutover plan: [`v1-to-v2-cutover.md`](./v1-to-v2-cutover.md).
...
Each item below has its own PRD in `plan/` with detailed scope, open questions, and risks:

- **[Hash URL migration](./v2.2-hash-url-migration.md)** — drop `#` from URLs via BrowserRouter + per-player HTML stubs.
- **[Hero-pool vs meta overlay](./v2.2-hero-pool-meta-overlay.md)** — layer OverFast community pickrate onto the Overview team hero-pool bar.
- **[Hero portraits](./v2.2-hero-portraits.md)** — surface OverFast hero portraits on the leaderboard / scatter / hero pool.
- **[OverFast player endpoints](./v2.2-overfast-player-endpoints.md)** — evaluate `/players/{id}/stats/*`; current recommendation is **skip**.

Other items still on the backlog without their own PRD yet:
