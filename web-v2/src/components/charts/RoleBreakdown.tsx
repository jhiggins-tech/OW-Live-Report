...
export default function RoleBreakdown({ playerId }: { playerId: string }) {
  const query = useQuery({
    queryKey: ['player', 'roleBreakdown', playerId],
    queryFn: () => fetchPlayerRoleBreakdown(playerId),
  });
    ...
    <div className="chart-wrap">
      <ResponsiveContainer>
        <BarChart data={data} margin={{ top: 12, right: 24, bottom: 8, left: 0 }}>
          ...
          />
          <Bar yAxisId="left" dataKey="kda" fill="var(--mint)" radius={[6, 6, 0, 0]} name="KDA" />
          <Bar yAxisId="right" dataKey="winRate" fill="var(--sky)" radius={[6, 6, 0, 0]} name="Win rate" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
